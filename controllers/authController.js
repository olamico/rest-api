...
exports.signin = async (req,res) => {
    ...
    try{ const {error, value} = signinSchema.validate({email, password}); 
...
exports.signout = async (req,res) =>{
    ...
    .status(200)
    .json({success: true, message: 'signd out successfully'});
};
...
exports.sendVerificationCode = async (req,res) => {
    ...
    if (existingUser.verified) {
        return res
        .status(400)
        .json({success: false, message: 'user You are already verified' });
    }
        const codeValue = Math.floor(Math.random() * 1000000).toString();
        let info = await transport.sendMail({
            from: process.env_NODE_CODE_SENDING_EMAIL_ADDRESS,
